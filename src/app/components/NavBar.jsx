  ...
  return (
    ...
    <div>
      <div className="flex justify-between items-center bg-[#31dce2] p-5">
        <Link href = "/">
        <button
          className="border-solid border-2 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-lg"
        >
        Cancel
        </button>
        </Link>
          ...
          {tabsData.map((tab, index) => {
            return (
              ...
              <Link href ={tab.links} key={index} className="w-full">
                <button key={index} className={`w-full py-4 text-lg border-b-4 transition-colors duration-300 
                ${index === activeTabIndex ? "border-teal-500" : "border-transparent hover:border-gray-200"}`}
